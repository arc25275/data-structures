...
# Assignment: Assignment 3 - Queue Static Array
# Due Date: 2/12/2024
# Description: Create a Queue class using a static array
...
class Queue:
    ...
    def dequeue(self) -> object:
        """Remove first value of queue and return it"""
        if self.is_empty():
    ...
    def _double_queue(self) -> None:
        ...
        j = 0
        while True:
            new_array[j] = self._sa[i]
            i = self._increment(i)
            j += 1
            if i == self._back:
                new_array[j] = self._sa[i]  # Copy the last element
                break
        self._sa = new_array
        self._front = 0
        self._back = j
