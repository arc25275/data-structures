...
# Due Date: 3/14/2024
# Description: Implementing a hash map with Separate Chaining
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        """Put a key and value pair into the hash map using the provided hash function"""
    ...
    def get(self, key: str):
        """Get a value from the map based on key given"""
        hash = self._hash_function(key)
        index = hash % self._capacity
        bucket = self._buckets[index]
...
def find_mode(da: DynamicArray) -> tuple[DynamicArray, int]:
    """Find the most occurring values in an unsorted array of values"""
    map = HashMap()
    for i in range(da.length()):
        if map.contains_key(da[i]):
            current_value = map.get(da[i])
            map.put(da[i], current_value + 1)
            # If value exists, increase count by 1
        else:
            map.put(da[i], 1)
    modes = DynamicArray()
    ...
    count = 0
    for i in range(pairs.length()):
        # for every pair, check if it is higher than current count
        key, value = pairs[i]
        if value > count:
            modes = DynamicArray()
            modes.append(key)
